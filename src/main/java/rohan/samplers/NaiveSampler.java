 ...
 */
import rohan.experiments.LogNumEquivMatricesTracker;
...
/** This class implements {@link Sampler} using the ROhAN-m method. */
public class NaiveSampler implements Sampler {
   ...
   * margins as the original dataset using the ROhAN-m method.
   *
  ...
  public SparseMatrix sample(SparseMatrix inMatrix, int numSwaps, long seed, Timer timer) {
    ...
    for (int i = 0; i < numSwaps; i++) {
      ...
      final double numEquivMatricesRatio =
          matrix.getNumEquivMatricesRatio(swappableRow1, swappableRow2, newRow1, newRow2);

      final double frac = numEquivMatricesRatio * ((double) matrixDegree / adjMatrixDegree);
