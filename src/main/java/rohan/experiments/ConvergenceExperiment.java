...
import rohan.mining.FreqItemsetMiner;
...
public class ConvergenceExperiment {
  ...
  private static void run(String confPath) {
    ...
    for (Sampler sampler : samplers) {
      ...
      for (int i = 0; i < numSwapsFactors.size(); i++) {
        ...
        final double avgRelFreqDiff = getAvgRelFreqDiff(freqItemsetToSup, sample, transformer);
        System.out.println("\t\t" + JsonKeys.avgRelFreqDiff + ": " + avgRelFreqDiff);

        final double perturbationScore = getPerturbationScore(realMatrix, sample);
        System.out.println("\t\t" + JsonKeys.perturbationScore + ": " + perturbationScore);
        ...
        factorConvergenceStats.put(JsonKeys.perturbationScore, perturbationScore);
  ...
  static double getAvgRelFreqDiff(
      Map<Set<Integer>, Integer> freqItemsetToSup, SparseMatrix sample, Transformer transformer) {
    System.out.println("\t\tGetting sample itemset to support map");
    Map<Set<Integer>, Integer> sampleItemsetToSup = getSampleItemsetToSupMap(sample, transformer,
            freqItemsetToSup.keySet());
    return getAvgRelFreqDiff(freqItemsetToSup, sampleItemsetToSup);
  }
  ...
  /**
   * Gets the perturbation score
   *
   * @param original the matrix representation of the original dataset
   * @param sample the matrix representation of the sampled dataset
   * @return the average fraction of cells differing from the corresponding ones of the initial matrix
   */
  static double getPerturbationScore(
         SparseMatrix original, SparseMatrix sample) {
    double sumDiffEntries = 0;

    for (int i = 0; i< original.getNumRows();i++){
      for (int j = 0; j < original.getNumCols(); j++){
        if (original.get(i, j) != sample.get(i, j)) {
          sumDiffEntries++;
        }
      }
    }

    return sumDiffEntries / (original.getNumRows()*original.getNumCols());
  }
